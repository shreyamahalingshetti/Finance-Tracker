  ...
  return (
    <div className="min-h-screen flex" style={{ background: 'linear-gradient(135deg, #f8f9ff 0%, #e5eeff 50%, #dce9ff 100%)' }}>
      ...
      <div className="hidden lg:flex lg:w-1/2 flex-col justify-between p-16 relative overflow-hidden"
        ...
        <div className="absolute top-1/2 right-12 w-32 h-32 rounded-full opacity-5" style={{ background: 'white' }} />
        ...
        <div className="relative z-10">
          ...
          <div className="space-y-8">
            <div>
              <h2 className="text-headline-lg text-white mb-4" style={{ fontSize: '40px', lineHeight: '48px' }}>
                Take control of your finances.
              </h2>
              <p className="text-body-lg text-white opacity-80">
                Track spending, visualize trends, and reach your goals — all in one beautiful dashboard.
              </p>
            </div>

            {/* Feature list */}
            {[
              { icon: 'insights', text: 'Automated spending insights' },
              { icon: 'pie_chart', text: 'Visual expense breakdowns' },
              { icon: 'savings', text: 'Goal tracking & alerts' },
            ].map((f) => (
              <div key={f.icon} className="flex items-center gap-4">
                <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0"
                  style={{ background: 'rgba(255,255,255,0.15)' }}>
                  <span className="material-symbols-outlined text-white" style={{ fontSize: '20px' }}>{f.icon}</span>
                </div>
                <p className="text-body-lg text-white opacity-90">{f.text}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        <div className="w-full max-w-md animate-fade-in">
          ...
          <div className="mb-8">
            <h2 className="text-headline-md mb-2" style={{ color: '#0b1c30' }}>Welcome back</h2>
            <p className="text-body-lg" style={{ color: '#464555' }}>Sign in to your account to continue.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            {/* Email */}
            <div>
              <label className="block text-label-md mb-2 tracking-wider" style={{ color: '#464555' }}>EMAIL ADDRESS</label>
              <div className="relative">
                <span className="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2"
                  style={{ color: '#777587', fontSize: '20px' }}>mail</span>
                <input
                  id="login-email"
                  type="email"
                  value={email}
          ...
          <div className="mt-8 text-center">
            <p className="text-body-md" style={{ color: '#464555' }}>
              ...
              <Link href="/register" className="font-bold transition-colors duration-150" style={{ color: '#3525cd' }}>
                Create one free
              </Link>
            </p>
          </div>
